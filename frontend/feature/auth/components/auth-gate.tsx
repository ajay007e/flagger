 ...
 * the current session (if any) is still valid, then either renders children
 * or blocks behind the login modal until the user logs in — no dedicated
 * /login page or redirect, so whatever the user was looking at is exactly
 * where they land afterward.
 *
 * A session that goes invalid *after* this check (e.g. it expires while
 * browsing) is caught by the axios interceptor instead, which flips the same
 * store directly.
 */
